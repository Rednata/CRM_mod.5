...
import {
  addSumProperty, getSumInTable}
  from './modules/calculate.js';
...
import {deleteItem} from './modules/controlTable.js';

import { functionForm } from './modules/functionForm.js';
...
const init = () => {
  ...
  addSumProperty(goods);
  getSumInTable(goods, sumInTable);
  renderGoods(goods, tableBody);
  clickEventAddGoodButton(vendorCodeID, addGoodButton, overlay);
  clickEventOverlay(overlay);
  deleteItem(goods, tableBody, sumInTable);
  functionForm(vendorCodeID, goods, tableBody, overlay, sumInTable);
};
